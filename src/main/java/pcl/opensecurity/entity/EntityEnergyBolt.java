...
public class EntityEnergyBolt extends Entity {
    ...
    public void setHeading(float yaw, float pitch) {
        ...
        // This will crash when run on a dedicated server, because Entity#setVelocity is marked with
        // @SideOnly(Side.CLIENT), and therefore stripped from a dedicated server instance.
        // setVelocity((float)(-Math.sin(yaw) * Math.cos(pitch)), (float)-Math.sin(pitch), (float)(Math.cos(yaw) *
        // Math.cos(pitch)));
