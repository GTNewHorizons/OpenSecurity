...
public class Config {
    ...
    public Config(Configuration config) {
        ...
        registerBlockBreak = config.getBoolean(
                "registerBlockBreak",
                "options",
                true,
        ...
        magCardDisplayName = config.getBoolean(
                ...
                "options",
                true,
                "If true - the Mag Card Reader will return the Minecraft Username of the player who swiped the card, false will return \"player\"");
        biggerEEPROM = config.getBoolean(
                "biggerEEPROMS",
                "options",
                false,
                "If enabled changes EEPROMS capacity to 8096 bytes (Only when flashed with the OS CardWriter!)");
        returnRealUUID = config.getBoolean(
                "returnRealUUID",
                "options",
                false,
                "If enabled makes the BioMetric reader return the user's REAL UUID, and not the base64'd hash");
