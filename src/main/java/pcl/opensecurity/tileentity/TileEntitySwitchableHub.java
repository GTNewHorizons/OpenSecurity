...
public class TileEntitySwitchableHub extends TileEntitySidedEnvironment implements SidedEnvironment, Environment {
    ...
    public static ForgeDirection getWorldSide(String localSide, ForgeDirection facing) {
        if ("up".equalsIgnoreCase(localSide)) return ForgeDirection.UP;
        if ("down".equalsIgnoreCase(localSide)) return ForgeDirection.DOWN;
        ...
        if ("south".equalsIgnoreCase(localSide)) return sideFront;
        if ("north".equalsIgnoreCase(localSide)) return sideBack;
        if ("east".equalsIgnoreCase(localSide)) return sideLeft;
        if ("west".equalsIgnoreCase(localSide)) return sideRight;
        throw new IllegalArgumentException("Invalid side");
    }
}
