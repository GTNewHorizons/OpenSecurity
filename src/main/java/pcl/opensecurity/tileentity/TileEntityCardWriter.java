...
public class TileEntityCardWriter extends TileEntityMachineBase implements Environment, IInventory, ISidedInventory {
    ...
    private void initOCFilesystem() {
        oc_fs = li.cil.oc.api.FileSystem.asManagedEnvironment(
                li.cil.oc.api.FileSystem.fromClass(OpenSecurity.class, OpenSecurity.MODID, "/lua/cardwriter/"),
                "cardwriter");
    ...
    public void updateEntity() {
        ...
        if (!hasCards && getStackInSlot(0) != null) {
            hasCards = true;
            if (node != null) node.sendToReachable("computer.signal", "cardInsert", "cardInsert");
        }

        if (hasCards && getStackInSlot(0) == null) {
            hasCards = false;
            if (node != null) node.sendToReachable("computer.signal", "cardRemove", "cardRemove");
        }
    }
