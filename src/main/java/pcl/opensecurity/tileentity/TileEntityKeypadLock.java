...
public class TileEntityKeypadLock extends TileEntityMachineBase implements Environment {
    ...
    public void readFromNBT(NBTTagCompound nbt) {
        ...
        for (int i = 0; i < 12; ++i) buttonLabels[i] = trimString(nbt.getString("btn:" + i), MAX_LABEL_LENGTH);
        byte[] colors = nbt.getByteArray("btn:colors");
        if (colors != null) for (int i = 0; i < 12 && i < colors.length; ++i) buttonColors[i] = colors[i];
    ...
    public void writeToNBT(NBTTagCompound nbt) {
        ...
        for (int i = 0; i < 12; ++i) nbt.setString("btn:" + i, buttonLabels[i]);
    ...
    public void pressedButton(EntityPlayer player, int buttonIndex) {
        ...
        if (!worldObj.isRemote) {
            ...
            node.sendToReachable(
                    ...
                    eventName,
                    buttonIndex + 1,
                    buttonIndex >= 0 && buttonIndex < buttonLabels.length ? buttonLabels[buttonIndex] : "");
        }
    }
