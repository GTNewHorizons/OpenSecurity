...
public class BlockLocation {
    ...
    public BlockLocation offset(ForgeDirection direction, int distance) {
        return relative(direction.offsetX * distance, direction.offsetY * distance, direction.offsetZ * distance);
    }
    ...
    /**
     * Gets the tile entity of the block at this location. Returns null if the tile entity is not the correct type.
     */
    ...
    /**
     * Gets the tile entity of the block at this location. Throws an error if there is no tile entity or it's not the
     * correct type.
     */
    @SuppressWarnings("unchecked")
    public <T extends TileEntity> T getTileEntityStrict(Class<T> tileEntityClass) {
        TileEntity tileEntity = getTileEntity();
        if (tileEntity == null) throw new Error(String.format("Expected tile entity at %s, but none found.", this));
        if (!tileEntityClass.isInstance(tileEntity)) throw new Error(
                String.format(
                        "Expected tile entity at %s to be '%s', but found '%s' instead.",
                        this,
                        tileEntityClass.getName(),
     ...
     * <b>Warning:</b> This is usually done automatically. Only use this when you know what you're doing!
     */
    ...
    public boolean equals(Object obj) {
        BlockLocation loc;
        return ((obj instanceof BlockLocation) && (blockAccess == (loc = (BlockLocation) obj).blockAccess)
                && (x == loc.x)
                && (y == loc.y)
                && (z == loc.z));
    }
    ...
    private String getWorldName() {
        return isWorld ? ("DIM=" + Integer.toString(world.provider.dimensionId)) : blockAccess.toString();
    }
}
